...
pub fn is_valid_placement(board: &[[i8;9];9], row: &usize, col: &usize, number: &i8) -> bool {
    if !is_in_box(board, row, col, number) && !is_in_column(board, col, number) && !is_in_row(board, row, number) {
        return true;
    }
    else {
        return false;
...
mod tests {
    ...
    #[test]
    fn solve_board_test() {
        let mut board1 = [[7,5,2,0,5,0,6,0,0],
                        [0,0,0,0,0,3,0,0,0],
                        [1,0,0,0,0,9,5,0,0],
                        [8,0,0,0,0,0,0,9,0],
                        [0,4,3,0,0,0,7,5,0],
        ...
        let mut board2 = [[7,0,2,0,5,0,6,0,0],
                        [0,0,0,0,0,3,0,0,0],
                        [1,0,0,0,0,9,5,0,0],
                        [8,0,0,0,0,0,0,9,0],
                        [0,4,3,0,0,0,7,5,0],
                        ...
                        [0,0,0,2,0,0,0,0,0],
                        [0,0,7,0,4,0,2,0,3]];
        assert_eq!(solve(&mut board1), false);
        assert_eq!(solve(&mut board2), true);
    }
}
